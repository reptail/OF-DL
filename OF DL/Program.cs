...
public class Program
{
	...
	public static async Task Main(string[] args)
	{
		...
		try
		{
			...
			{
                await File.WriteAllTextAsync("config.json", JsonConvert.SerializeObject(new Entities.Config(), Formatting.Indented));
			...
			switch (mainMenuSelection)
			{
				...
				case "[red]Change logging level[/]":
					while (true)
					{
						...
						string newConfigString = JsonConvert.SerializeObject(newConfig, Formatting.Indented);
                        await File.WriteAllTextAsync("config.json", newConfigString);
